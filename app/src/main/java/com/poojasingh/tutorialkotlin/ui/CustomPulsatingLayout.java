...
public class CustomPulsatingLayout extends RelativeLayout {
    ...
    private ArrayList<CustomPulsatingLayout.RippleView> rippleViewList = new ArrayList<>();

    public CustomPulsatingLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    public CustomPulsatingLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(context, attrs);
    }

    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    public CustomPulsatingLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initializeAnimators(final LayoutParams params, final int amount, final float scale, final int delay, final int durationTime) {
        ...
        for (int i = 0; i < amount; i++) {
            CustomPulsatingLayout.RippleView rippleView = new CustomPulsatingLayout.RippleView(getContext());
    ...
    private ObjectAnimator animateScaleX(final CustomPulsatingLayout.RippleView view, final float scale, final int delay, final int durationTime, final int index) {
    ...
    private ObjectAnimator animateScaleY(final CustomPulsatingLayout.RippleView view, final float scale, final int delay, final int durationTime, final int index) {
    ...
    private ObjectAnimator animateAlpha(final CustomPulsatingLayout.RippleView view, final int delay, final int durationTime, final int index) {
    ...
    public void startAnimation() {
        if (!isRippleAnimationRunning()) {
            for (CustomPulsatingLayout.RippleView rippleView : rippleViewList) {
